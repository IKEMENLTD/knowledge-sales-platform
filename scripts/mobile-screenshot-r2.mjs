...
const OUT = 'C:/tmp/ksp-ux-r2-mobile';
...
for (const vp of viewports) {
  ...
  // viewport-fit shot (no fullPage, to verify Dialog max-h would respect 85svh visible area)
  for (const p of pages) {
    try {
      await page.goto(p.url, { waitUntil: 'networkidle', timeout: 20000 });
      await page.waitForTimeout(400);
      await page.screenshot({ path: `${OUT}/${vp.name}-${p.name}-viewport.png`, fullPage: false });
      console.log(`OK ${vp.name}-${p.name}-viewport`);
    } catch (e) {
      console.log(`FAIL ${vp.name}-${p.name}-viewport: ${e.message}`);
    }
  }
  await ctx.close();
}
